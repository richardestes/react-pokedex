...
const Pokedex = (props) => {
  ...
  useEffect(() => {
    axios.get(`https://pokeapi.co/api/v2/pokemon?limit=807`).then(function (response) {
      //api currently only has images up to pokemon #807
      const { data } = response;
      const { results } = data;
      const newPokemonData = {};
      results.forEach((pokemon, index) => {
        let adjustedIndex = index + 1; //index offset by 1 since pokemon ids start at 1
        newPokemonData[adjustedIndex] = {
          id: adjustedIndex,
          name: pokemon.name,
          sprite: `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/${adjustedIndex}.png`,
        };
      });
